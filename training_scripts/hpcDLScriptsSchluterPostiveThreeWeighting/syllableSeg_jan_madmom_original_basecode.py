...
def syllableSeg_jan_madmom_original_basecode(part, ii):
    ...
    feature_all, label_all, sample_weights_all, scaler = concatenateFeatureLabelSampleweights(train_fns,
                                                                                              ...
                                                                                              nlen=7,
                                                                                              scaling=True)
    filename_train_validation_set = join(schluter_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'feature_all_jan_temp_'+str(ii)+'.h5')
    filename_labels_train_validation_set = join(schluter_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'labels_train_set_all_jan_temp_'+str(ii)+'.pickle.gz')
    filename_sample_weights = join(schluter_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'sample_weights_all_jan_temp_'+str(ii)+'.pickle.gz')
    filename_scaler = join(schluter_feature_data_path_madmom_positiveThreeSampleWeighting, 'temp', 'scaler_jan_madmom_positiveThreeSampleWeighting_early_stopping_'+str(ii)+'.pickle.gz')
