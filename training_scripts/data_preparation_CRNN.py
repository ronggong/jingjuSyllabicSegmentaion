...
def featureLabelSampleWeightsLoad(data_path, filename, scaler):
    ...
    label_path = os.path.join(data_path, 'label' + '_' + filename + '.pickle.gz')
    sample_weights_path = os.path.join(data_path, 'sample_weights' + '_' + filename + '.pickle.gz')

    f = h5py.File(mfcc_line_path, 'r')
    mfcc_line = f['feature_all']
    label = cPickle.load(gzip.open(label_path, 'r'))
    sample_weights = cPickle.load(gzip.open(sample_weights_path, 'r'))
