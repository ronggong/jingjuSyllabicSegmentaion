...
def syllableSeg_jordi_madmom_basecode(part, ii, model_name='jordi_timbral_schluter'):
    ...
    feature_all, label_all, sample_weights_all, scaler = concatenateFeatureLabelSampleweights(train_fns,
                                                                                              ...
                                                                                              nlen=7,
                                                                                              scaling=True)
    filename_train_validation_set = join(schluter_feature_data_path_madmom_simpleSampleWeighting, 'temp', 'feature_all_'+model_name+'_temp_'+str(ii)+'.h5')
    filename_labels_train_validation_set = join(schluter_feature_data_path_madmom_simpleSampleWeighting, 'temp', 'labels_train_set_all_'+model_name+'_temp_'+str(ii)+'.pickle.gz')
    filename_sample_weights = join(schluter_feature_data_path_madmom_simpleSampleWeighting, 'temp', 'sample_weights_all_'+model_name+'_temp_'+str(ii)+'.pickle.gz')
    filename_scaler = join(schluter_feature_data_path_madmom_simpleSampleWeighting, 'temp', 'scaler_'+model_name+'_madmom_simpleSampleWeighting_early_stopping_'+str(ii)+'.pickle.gz')
