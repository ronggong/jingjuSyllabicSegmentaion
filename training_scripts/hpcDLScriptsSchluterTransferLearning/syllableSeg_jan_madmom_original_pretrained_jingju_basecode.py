...
def syllableSeg_jan_madmom_original_basecode(part, ii, deep=False, dense=False):
    ...
    filename_sample_weights = join(schluter_feature_data_path_madmom_simpleSampleWeighting, 'temp', 'sample_weights_all_jan_pretrained_jingju_temp_'+str(ii)+'.pickle.gz')
    filename_scaler = join(schluter_feature_data_path_madmom_simpleSampleWeighting, 'temp', 'scaler_jan_madmom_simpleSampleWeighting_early_stopping_schluter_pretrained_jingju_dataset_'+str(ii)+'.pickle.gz')
